...
import { ArrowRight, MapPin, Users, Shield, Zap, ChevronDown } from 'lucide-react'
...
function ManifestoSection() {
  ...
  return (
    <section className="min-h-[60vh] flex items-center justify-center py-20 px-4 bg-stone-950">
          ...
          return (
            <p
              ...
              className={`font-display transition-all duration-700 ${
                ...
                isKey
                  ? 'text-2xl md:text-3xl text-gather-400 font-semibold'
                  : i < 3
  ...
  const steps = [
    {
      ...
      desc: 'Your city or neighborhood. That\'s all we need. No tracking, no data profiles.',
    },
    {
      number: '02',
      icon: '🎯',
      title: 'Pick what you love doing',
      desc: 'Hiking, cooking, board games, music, yoga — activities you\'d actually enjoy.',
    },
    {
      number: '03',
      icon: '🙋',
      title: 'Join or host a gathering',
      desc: 'Small groups (max 8–12). Real places. Scheduled times. You actually show up.',
    },
    {
      number: '04',
      icon: '🤝',
      title: 'Meet people, for real',
      desc: 'No DMs, no followers, no feeds. Just a table, a trail, a kitchen, a room.',
    },
  ]
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16">
          ...
          <h2 className="font-display text-4xl md:text-5xl text-stone-900">
            Four steps to an actual friendship
          </h2>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {steps.map((step, i) => (
            <div key={i} className="relative">
              {i < steps.length - 1 && (
                <div className="hidden lg:block absolute top-8 left-[calc(100%+12px)] w-6 text-stone-200 text-2xl font-mono">
                  →
                </div>
              )}
              <div className="bg-cream-50 rounded-3xl p-6 h-full border border-cream-200">
...
function Principles() {
  const principles = [
    {
      icon: <Users className="text-sage-500" size={22} />,
      title: 'Small by design',
      desc: 'Max 8–12 people per gathering. Big enough to meet someone new. Small enough to actually talk.',
      bg: 'bg-sage-50',
      border: 'border-sage-100',
    },
    {
      icon: <Shield className="text-gather-500" size={22} />,
      title: 'No algorithm, ever',
      desc: 'No feed, no engagement optimization, no "suggested for you." You see what\'s near you, that\'s it.',
      bg: 'bg-gather-50',
      border: 'border-gather-100',
    },
    {
      icon: <MapPin className="text-blue-500" size={22} />,
      title: 'Radically local',
      desc: 'Everything within walking or short driving distance. No global broadcasts. Your actual community.',
      bg: 'bg-blue-50',
    ...
    },
    {
      ...
      desc: 'You\'re not just "hanging out" — you\'re hiking, cooking, drawing, playing games. Shared doing creates real bonds.',
      bg: 'bg-amber-50',
      ...
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-14">
          ...
          <p className="mt-4 text-lg text-stone-500 max-w-xl mx-auto">
            Every design decision we make is the opposite of what makes social media addictive.
            That's intentional.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {STATS.map((stat, i) => (
            <div key={i} className="text-center">
              ...
              <div className="font-display text-3xl lg:text-4xl font-bold text-gather-400 mb-1">
                {stat.value}
              </div>
...
function Testimonials() {
  const quotes = [
    {
      quote: "I moved to Portland knowing no one. Six months of Gather later, I have a hiking crew, a board game night, and two people I'd call if something went wrong.",
      author: "Dani L.",
      detail: "Joined 6 months ago · 23 gatherings attended",
      avatar: "DL",
    },
    {
      quote: "I was skeptical — I'm 58 and tech isn't my thing. But my neighbor convinced me to try a Repair Café. I've been back every month. People fix things together, you know?",
      author: "Robert K.",
      detail: "Joined 14 months ago · 8 gatherings attended",
      avatar: "RK",
    },
    {
      quote: "After my divorce I felt invisible. The cooking group changed that. We're not just chopping vegetables together — we're actually talking. Really talking.",
      author: "Sarah M.",
      detail: "Joined 4 months ago · 11 gatherings attended",
      avatar: "SM",
    },
  ]
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {quotes.map((q, i) => (
            <div key={i} className="bg-white rounded-3xl p-7 border border-cream-200 shadow-sm flex flex-col gap-4">
              <p className="font-display text-lg text-stone-800 leading-relaxed italic">
                &ldquo;{q.quote}&rdquo;
              </p>
              <div className="mt-auto flex items-center gap-3 pt-4 border-t border-cream-100">
  ...
  return (
    <section className="py-28 px-4 bg-gather-500">
      <div className="max-w-3xl mx-auto text-center">
        ...
        <p className="mt-8 text-gather-200 text-sm">
          No ads. No engagement bait. No selling your data. Ever.
        </p>
      </div>
    </section>
  )
  ...
  return (
    <footer className="bg-stone-950 text-stone-400 py-16 px-4">
      <div className="max-w-5xl mx-auto grid md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          ...
          <p className="mt-4 text-xs text-stone-600">
            A Public Benefit Corporation. We exist to create community, not shareholder value.
          </p>
        </div>
        <div>
          <div className="text-xs font-bold uppercase tracking-widest text-stone-600 mb-4">Product</div>
          <ul className="space-y-2 text-sm">
            {['Discover', 'Host a gathering', 'How it works', 'Safety'].map(l => (
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {['Why we exist', 'Our principles', 'Press', 'Contact'].map(l => (
              <li key={l}><a href="#" className="hover:text-white transition-colors">{l}</a></li>
            ))}
          </ul>
        </div>
      ...
      <div className="max-w-5xl mx-auto mt-12 pt-6 border-t border-stone-800 flex flex-wrap items-center justify-between gap-4 text-xs text-stone-600">
        <span>© 2026 Gather PBC. Made with care in Portland, OR.</span>
        <span>Privacy · Terms · No cookies except functional ones.</span>
      </div>
    </footer>
  )
}
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-cream-50 to-white py-24 md:py-32 px-4">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <p className="text-xl md:text-2xl text-stone-500 max-w-2xl mx-auto mb-10 leading-relaxed">
            Find real people doing real things near you — hiking, cooking, making art, playing games.
          ...
          <p className="text-sm text-stone-400 flex items-center justify-center gap-2">
            ...
            Free forever. Open to everyone. No account needed to browse.
          </p>
        </div>
